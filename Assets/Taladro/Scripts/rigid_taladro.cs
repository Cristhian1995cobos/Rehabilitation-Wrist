using System.Collections;
using UnityEngine;

public class rigid_taladro : MonoBehaviour {


    public float k = 0.5f;
    public float mass = 1;
    public Vector3 device_scale = new Vector3(1.0f, 1.0f, 1.0f);//send 1/4 size in the z dimension
    public int bodyId = 0;
    public Vector3 linearFactors = new Vector3(0.0f, 0.0f, 1.0f);
    public Vector3 angularFactors = new Vector3(0.0f, 0.0f, 0.0f);
    public float friction = 0.8f;
    private float time1 = 0;


    // Use this for initialization
    void Start()
    {
        bodyId = getNextBodyId1();

        refreshShape();
       
    }

    public void refreshShape()
    {
        Mesh m = GetComponent<MeshFilter>().mesh;
        Vector3[] v = m.vertices;
        int[] f = m.triangles;
        float[] shape = new float[f.Length * 3];

        for (int i = 0; i < f.Length; i++)
        {
            Vector3 vert = v[f[i]];
            vert.Scale(transform.localScale);
            vert.Scale(device_scale);
            //				vert = transform.localRotation*vert;

            shape[i * 3] = vert.x;
            shape[i * 3 + 1] = vert.y;
            shape[i * 3 + 2] = vert.z;
        }
        Vector3 localPosition = transform.localPosition;
        localPosition.Scale(device_scale);


        FalconUnity.sendDynamicShape(bodyId, shape, f.Length / 3, mass, k, localPosition, transform.localRotation, linearFactors, angularFactors, friction);
    }

    private void Update()
    {
        time1 += Time.deltaTime;
        if (time1 < 10)
        {
            linearFactors = new Vector3(0, 0, 0);
            angularFactors = new Vector3(0, 0, 0);
        }

    }

    // Update is called once per frame
    public void FixedUpdate()
    {
        Vector3 pos;
        Quaternion orient;
        if (time1 > 10)
        {


            if (tag == "piezas")
            {
                if (contador.broca == 0)
                {

                    linearFactors = new Vector3(0, 0, 0);
                    //  Debug.Log("pared");

                }
                else
                {
                    linearFactors = new Vector3(0, 0, 1);
                    // Debug.Log("pared2");
                }

            }
            if (tag == "piezas2")
            {
                if (contador.broca == 1)
                {
                    linearFactors = new Vector3(0, 0, 0);
                    //  Debug.Log("pared");

                }
                else
                {
                    linearFactors = new Vector3(0, 0, 1);

                }

            }
        }
            bool res = FalconUnity.getDynamicShapePose(bodyId, out pos, out orient);
            if (!res)
            {
                //			Debug.Log("Error getting object pose");
                return;
            }
            transform.localPosition = pos;
            transform.localRotation = orient;
            FalconUnity.updateDynamicShape(bodyId, mass, k, linearFactors, angularFactors, friction);
        
    }

    static object Lock1 = new object();
    private static int curId1 = -1;
    public static int getNextBodyId1()
    {
        lock (Lock1)
        {
            curId1++;
            return curId1;
        }
    }
}
